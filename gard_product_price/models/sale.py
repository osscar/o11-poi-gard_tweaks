...
class SaleOrderLine(models.Model):
    ...
    pricelist_item_count = fields.Integer(
        ...
        compute="_get_count_active_pricelist_items"
    )
    ...
    def _get_display_price(self, product):
        ...
        if self.pricelist_id:
            ...
            final_price, rule_id = self.pricelist_id.with_context(product_context).get_product_price_rule(self.product_id, self.product_uom_qty or 1.0, self.order_id.partner_id)
            base_price, currency_id = self.with_context(product_context)._get_real_price_currency(product, rule_id, self.product_uom_qty, self.product_uom, self.pricelist_id.id)
