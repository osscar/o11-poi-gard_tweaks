...
class SiatCancelInvoice(models.TransientModel):
    ...
    def siat_cancel_invoice(self):

        # self.ensure_one()
        # invoice_obj = self.env["account_invoice"]
        invoices_cancel = self.wizard_line

        for inv in invoices_cancel:
            if inv.type == "manual":
            
            base_dosif = inv.dosif_id
        ...
        response = SiatService(caso_siat_fac, self.env).service(method, SolicitudServicioAnulacionFactura)
        ...
        if not transaccion:
            if siat_codigo == 123:
                ...
                raise UserError('[SIAT] No se ha podido anular la factura en el SIAT: \n %s' % (str(mensajesList),))
        else:
